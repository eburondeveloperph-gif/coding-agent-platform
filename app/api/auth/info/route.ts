...
export async function GET(req: NextRequest) {
  try {
    ...
    // For GitHub/local users, return the existing session without recreating it.
    // For Vercel users, recreate the session to refresh user data.
    let session: Session | undefined
    if (existingSession && (existingSession.authProvider === 'github' || existingSession.authProvider === 'local')) {
    ...
    console.error('Auth info route failed', error)
