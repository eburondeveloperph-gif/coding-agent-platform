...
import { canonicalizeStoredAgent, getDefaultModelForAgent, getDisplayAgentName } from '@/lib/openmax/agents'
import { BRAND_GIT_AUTHOR_NAME } from '@/lib/brand'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const selectedAgent = canonicalizeStoredAgent(validatedData.selectedAgent)
    const selectedModel = validatedData.selectedModel || getDefaultModelForAgent(selectedAgent)
    ...
    after(async () => {
      try {
        ...
        const aiBranchName = await generateBranchName({
          ...
          repoName,
          context: `${getDisplayAgentName(selectedAgent)} agent task`,
        })
...
async function processTask(
  ...
  const canonicalSelectedAgent = canonicalizeStoredAgent(selectedAgent)
  const selectedModelToUse = selectedModel || getDefaultModelForAgent(canonicalSelectedAgent)
  ...
  try {
    ...
    const sandboxResult = await createSandbox(
      {
        ...
        repoUrl,
        githubToken,
        gitAuthorName: githubUser?.name || githubUser?.username || BRAND_GIT_AUTHOR_NAME,
        ...
        resources: { vcpus: 4 },
        taskPrompt: prompt,
        selectedAgent: canonicalSelectedAgent,
        selectedModel: selectedModelToUse,
        installDependencies,
        keepAlive,
        enableBrowser,
    ...
    const agentResult = await executeAgentInSandbox(
      sandbox,
      sanitizedPrompt,
      canonicalSelectedAgent as AgentType,
      logger,
      selectedModelToUse,
      mcpServers,
      undefined,
      ...
      try {
        ...
        if (process.env.AI_GATEWAY_API_KEY) {
          commitMessage = await generateCommitMessage({
            description: prompt,
            repoName,
            context: `${getDisplayAgentName(canonicalSelectedAgent)} agent task`,
          })
        } else {
