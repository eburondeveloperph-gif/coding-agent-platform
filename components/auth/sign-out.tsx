...
import { Key, LogOut, Server } from 'lucide-react'
...
export function SignOut({ user, authProvider }: Pick<Session, 'user' | 'authProvider'>) {
  ...
  const avatarUrl = user?.avatar ? `${user.avatar}${user.avatar.includes('?') ? '&' : '?'}s=72` : undefined
  ...
  return (
    <DropdownMenu onOpenChange={(open) => open && fetchRateLimit()}>
      ...
      <DropdownMenuContent align="end" className="w-56">
        ...
        {/* Only show GitHub Connect/Disconnect for non-GitHub users when GitHub is enabled */}
