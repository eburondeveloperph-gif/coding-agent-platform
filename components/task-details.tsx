...
const CODING_AGENTS = [
  { value: 'codemax', label: 'eburonmax', icon: Codex },
  { value: 'terminal', label: 'terminal', icon: Gemini },
  { value: 'orbit', label: 'orbit', icon: Claude },
  { value: 'echo', label: 'echo', icon: OpenCode },
  { value: 'vision', label: 'vision', icon: Cursor },
] as const
...
export function TaskDetails({ task, maxSandboxDuration = 300 }: TaskDetailsProps) {
  ...
  const [selectedAgent, setSelectedAgent] = useState(canonicalizeStoredAgent(task.selectedAgent || 'codemax'))
  const [selectedModel, setSelectedModel] = useState<string>(
    task.selectedModel || getDefaultModelForAgent(task.selectedAgent || 'codemax'),
  )
  ...
  const getModelName = (modelId: string | null, agent: string | null) => {
    if (!modelId || !agent) return 'model'

    const agentModels = getModelsForAgent(agent)
  ...
  useEffect(() => {
    if (selectedAgent) {
      const normalizedAgent = canonicalizeStoredAgent(selectedAgent)
      const agentModels = getModelsForAgent(normalizedAgent)
      const defaultModel = getDefaultModelForAgent(normalizedAgent)
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                onClick={() => {
                  ...
                  if (task.selectedAgent) params.set('agent', canonicalizeStoredAgent(task.selectedAgent))
          ...
          <div className="py-4 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  value={selectedAgent}
                  onValueChange={(value) => setSelectedAgent(canonicalizeStoredAgent(value))}
                >
