...
const CODING_AGENTS = [
  ...
  { value: 'codemax', label: 'eburonmax', icon: Codex, isLogo: true },
  { value: 'terminal', label: 'terminal', icon: Gemini, isLogo: true },
  { value: 'orbit', label: 'orbit', icon: Claude, isLogo: true },
  { value: 'echo', label: 'echo', icon: OpenCode, isLogo: true },
  { value: 'vision', label: 'vision', icon: Cursor, isLogo: true },
] as const

const normalizeTaskFormAgent = (agent?: string | null): string => {
  if (agent === 'multi-agent') {
    return 'multi-agent'
  }
  return canonicalizeStoredAgent(agent || 'codemax')
}
...
export function TaskForm({
  ...
  const [selectedAgent, setSelectedAgent] = useState(normalizeTaskFormAgent(savedAgent || 'codemax'))
  const [selectedModel, setSelectedModel] = useState<string>(getDefaultModelForAgent('codemax'))
  ...
  useEffect(() => {
    ...
    if (
      urlAgent &&
      CODING_AGENTS.some(
        (agent) => agent.value === normalizeTaskFormAgent(urlAgent) && !('isDivider' in agent && agent.isDivider),
      )
    ) {
      const normalizedUrlAgent = normalizeTaskFormAgent(urlAgent)
      setSelectedAgent(normalizedUrlAgent)
      if (urlModel) {
        const agentModels = normalizedUrlAgent === 'multi-agent' ? [] : getModelsForAgent(normalizedUrlAgent)
      ...
      const normalizedSavedAgent = normalizeTaskFormAgent(savedAgent)
      if (
        CODING_AGENTS.some(
          (agent) => agent.value === normalizedSavedAgent && !('isDivider' in agent && agent.isDivider),
        )
      ) {
        setSelectedAgent(normalizedSavedAgent)
      }
    }
                  ...
                  <Select value="multi-select" onValueChange={() => {}} disabled={isSubmitting}>
                    ...
                    <SelectContent>
                      {CODING_AGENTS.filter(
                        (agent) => agent.value !== 'multi-agent' && !('isDivider' in agent && agent.isDivider),
                      ).map((agent) => (
                        <div key={agent.value}>
                          <div className="px-2 py-1.5 text-xs font-semibold text-muted-foreground">{agent.label}</div>
                          {getModelsForAgent(agent.value).map((model) => {
