...
export const users = pgTable(
  'users',
  {
    ...
    passwordHash: text('password_hash'), // Local auth password hash
    // Profile info
...
export const insertUserSchema = z.object({
  ...
  provider: z.enum(['github', 'vercel', 'local']),
...
export const tasks = pgTable('tasks', {
  ...
  selectedAgent: text('selected_agent').default('codemax'),
...
export const keys = pgTable(
  'keys',
  {
    ...
    userId: text('user_id')
      ...
      .references(() => users.id, { onDelete: 'cascade' }), // Foreign key to users table
    provider: text('provider', {
      enum: ['anthropic', 'openai', 'cursor', 'gemini', 'aigateway', 'ollama'],
    }).notNull(),
...
export const insertKeySchema = z.object({
  id: z.string().optional(),
  userId: z.string(),
  provider: z.enum(['anthropic', 'openai', 'cursor', 'gemini', 'aigateway', 'ollama']),
